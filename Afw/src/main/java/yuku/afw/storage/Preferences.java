...
public class Preferences {
    ...
    public static Set<String> getStringSet(Enum<?> key) {
        SharedPreferences pref = read();
        return pref.getStringSet(key.toString(), null);
    }

    public static void setStringSet(Enum<?> key, Set<String> values) {
        SharedPreferences pref = read();
        getEditor(pref).putStringSet(key.toString(), values);
        commitIfNotHeld();
    }
    ...
    private synchronized static SharedPreferences read() {
        ...
        if (dirty || cache == null) {
            ...
            if (BuildConfig.DEBUG)
                Log.d(TAG, "Preferences was read from disk in " + (SystemClock.uptimeMillis() - start) + " ms");
            dirty = false;
