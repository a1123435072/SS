...
public class CircleProgressView extends View {
    ...
    //FIXME test
    private int mSpinnerColorEnd1 = 0X002b9cf9;
    private int mSpinnerColorEnd2 = 0X602b9cf9;
    private int mSpinnerColorEnd3 = 0Xa12b9cf9;
...
//        theta += Math.PI/2.0;
    ...
    private void drawSpinner(Canvas canvas) {
        ...
        // FIXME test
        int[] colors = new int[]{mSpinnerColorEnd1, mSpinnerColorEnd2, mSpinnerColor, mSpinnerColor, mSpinnerColor};
        mBarSpinnerPaint.setShader(new SweepGradient(mCircleBounds.centerX(), mCircleBounds.centerY(), colors, null));
        Matrix matrix = new Matrix();
        mBarSpinnerPaint.getShader().getLocalMatrix(matrix);

        matrix.postTranslate(-mCircleBounds.centerX(), -mCircleBounds.centerY() - 10);
        matrix.postRotate(startAngle);
        matrix.postTranslate(mCircleBounds.centerX(), mCircleBounds.centerY() - 10);
        mBarSpinnerPaint.getShader().setLocalMatrix(matrix);
//        mBarSpinnerPaint.setColor(colors[1]);
