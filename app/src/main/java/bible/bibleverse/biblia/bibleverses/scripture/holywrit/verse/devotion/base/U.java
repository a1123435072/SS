...
import bible.bibleverse.biblia.bibleverses.scripture.holywrit.verse.devotion.base.storage.NoBackupSharedPreferences;
...
public class U {
	...
	@Nullable public static String removeSpecialCodes(@Nullable final String text, final boolean force) {
		...
		if (!force && text.charAt(0) != '@') return text;//真 && index0不位为@

		final StringBuilder sb = new StringBuilder(text.length());
		int pos = 0;

		while (true) {
			final int p = text.indexOf('@', pos); //返回次字符串第一次出现的的索引，从pos开始
			if (p == -1) {
				break;
			}

			sb.append(text, pos, p);  //将文字进行拼接开头的号码
			pos = p + 2;
