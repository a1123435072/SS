package bible.bibleverse.biblia.bibleverses.scripture.holywrit.verse.devotion.base.sv;
...
public class VersionConfigUpdaterService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        if (intent != null) {
            ...
            try {
                App.getLbm().sendBroadcast(new Intent(VersionsActivity.ACTION_UPDATE_REFRESHING_STATUS).putExtra(VersionsActivity.EXTRA_refreshing, true));
                handleCheckUpdate(auto);
            } finally {
                App.getLbm().sendBroadcast(new Intent(VersionsActivity.ACTION_UPDATE_REFRESHING_STATUS).putExtra(VersionsActivity.EXTRA_refreshing, false));
            }
        }
    }
    ...
    void handleCheckUpdate(final boolean auto) {
        ...
        try {
            ...
            modifyTimeBody = App.downloadString(Constants.API_BUSINESS_URL + "/book/check");
        } catch (IOException e) {
