package bible.bibleverse.biblia.bibleverses.scripture.holywrit.verse.devotion.base.config;
...
public class VersionConfig {
    ...
    private static final List<String> blacklist = Arrays.asList(new String[]{"English Standard Version"});
    ...
    private static VersionConfig convertConfig(VersionConfigJson root) {
        ...
        for (PresetJson presetJson : root.presets) {
            if(presetJson == null || presetJson.longName == null){
                continue;
            }
            if(blacklist.contains(presetJson.longName)){
                continue;
            }
...
//			preset.download_url = root.download_url_format.replace("$PRESET_NAME", presetJson.preset_name);
            preset.download_url = root.download_url_format + presetJson.preset_name;
