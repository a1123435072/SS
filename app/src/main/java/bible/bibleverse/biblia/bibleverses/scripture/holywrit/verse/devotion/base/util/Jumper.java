...
public class Jumper {
        ...
        notosis:
        {
            ...
            if (m.matches()) {
                if (BuildConfig.DEBUG)
                    logger.d("jumper stage 7: ref matching osis pattern found: " + osisId);
                ...
                if (BuildConfig.DEBUG)
                    logger.d("jumper stage 7: successfully parsed osis id: " + p_bookIdFromOsis + ' ' + p_chapter + ' ' + p_verse);
                return true;
            }
        }
    ...
    private int guessBook(List<BookRef> refs) {
        ...
        int pos_forLater = -1;
        {
            ...
            if (passed == 1) {
                if (BuildConfig.DEBUG)
                    logger.d("guessBook phase 2 success: " + pos_forLater + " for " + p_book);
                return pos_forLater;
            } else {
        ...
        if (p_book.length() >= 2) {
            ...
            if (pos != -1) {
                if (BuildConfig.DEBUG)
                    logger.d("guessBook phase 3 success: " + pos + " with score " + minScore);
                return pos;
            }
        }
        ...
        if (pos_forLater != -1) {
            if (BuildConfig.DEBUG)
                logger.d("guessBook phase 7 success: " + pos_forLater + " for " + p_book);
            return pos_forLater;
        }
